...
**No goalpost-moving (added 2026-04-09 after naval-scribe Command Address Book deadlock):** When an angle's objection on a feature has been resolved or overruled by the project owner, the SAME angle may NOT raise a NEW objection on the SAME feature at the next gate retry unless the new objection cites NEW evidence not present in the original review. This prevents infinite-objection loops where an angle simply rephrases its objection after being overruled. Specifically: if attempt N's `council_<gate>.json` for an angle was OBJECT and attempt N+1 from the same angle is also OBJECT, the orchestrator must compare the two `reason` fields. If they describe the same underlying concern (even with different framing), the second objection is auto-downgraded to advisory. Concrete example: SECURITY first objected that localStorage entries needed encryption; after that was resolved with a user-warning, SECURITY pivoted to attacking the host CSP architecture as "critical XSS" — same angle, same feature, completely different reason, no new evidence. That pivot is a goalpost move and not allowed.
