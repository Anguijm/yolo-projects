...
### uuid-inspector (2026-04-05)
- **KEEP**: `BigInt('0x' + timeHi + timeMid + timeLow)` for v1 60-bit Gregorian timestamp — cleaner than any Number-based approach. `/ 10000n` to get ms. The Gregorian offset `122192928000000000n` is the canonical constant. BigInt is safe in all modern browsers (Chrome 67+, FF 68+, Safari 14+).
- **KEEP**: `parseInt(raw.slice(0,12), 16)` for v7 Unix ms — 48 bits, max year 10889, well within `Number.MAX_SAFE_INTEGER`. No BigInt needed.
- **KEEP**: `currentUUID` module-level variable + delegated `click` listener on `elResult` — avoids re-querying `getElementById('copy-uuid-btn')` after every innerHTML update. Pair with `e.target.id === 'copy-uuid-btn'` check. Clean and zero-allocation per render.
- **KEEP**: Flat object `NS = { 'uuid-string': 'Name' }` for namespace lookup — one line of code replaces four `if` branches. Extensible. Pattern: static lookup table over if/else chains for enum-like mappings.
- **KEEP**: `new RegExp('^[0-9a-f]{8}-[0-9a-f]{4}...', 'i')` — no regex literal, no `"` or `//` risk. Passing `'i'` flag handles uppercase UUIDs. Always use `new RegExp(string, flags)` for UUID/similar validators.
- **KEEP**: Color-coded structure bar with legend — immediately clarifies which bits carry timestamp vs random vs version/variant. The legend uses the same CSS color classes as the bar spans, so they always stay in sync. High-value for "what does this field mean?" questions.
- **KEEP**: `b[10] = b[10] | 0x01` to set multicast (I/G) bit for synthetic v1 nodes — this correctly signals a random/privacy node per IEEE 802. All generated v1 UUIDs are therefore distinguishable from real MAC-bearing ones.
- **IMPROVE**: CSS classes `.f-ver` and `.f-var` were used in the struct bar HTML but never defined in CSS. Caught by self-audit after initial build. Pre-build checklist: for every CSS class referenced in JS template strings, verify it exists in the `<style>` block.
- **IMPROVE**: SW registration script was omitted from the initial build — only manifest was added. Caught by self-audit. Pre-build checklist: SW + manifest + registration script are three separate things — all three must be present for offline/PWA.
- **INSIGHT**: For "decode this opaque value" tools (UUID, JWT, timestamp), the most valuable display is the reverse-decode with relative time. Developers mostly want "how old is this?" not the raw hex. Prioritize `Relative` row with cyan accent.
- **INSIGHT**: The `[a-an-grammar]` eval_bugs pattern false-positives on field names like `rand_a` — the `a` in a compound token is not the article "a". Always self-audit: read the actual line before deciding it's a real bug.
- **COUNCIL**: Gemini MCP unavailable — internal 6-angle review performed. Fixes: added `.f-ver`/`.f-var` CSS, added struct bar legend, added Share button with URL hash. PARTIAL REVIEW logged.
