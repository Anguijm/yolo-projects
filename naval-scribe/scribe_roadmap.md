...
- [x] **Letter Quality Checker** (approved 2026-04-25, priority 1; shipped 2026-06-26) — "Check" button opens side panel scanning current form against 7 naval formatting rules (date format, SSIC pattern, Subject length, From/To non-empty, body para 1 starts with "1.", signature block, classification set). ✓/✗ per check with brief explanation. Advisory only — does not block export. Pure-DOM render (no innerHTML).
- [x] **Portable Draft Export/Import** (approved 2026-04-25, priority 2; shipped 2026-06-26) — "Export Draft" downloads current form state as `.navalscribe.json` (same shape as `getFullState`); "Import Draft" file picker reads it back via `restoreFullState`. Enables backup, cross-device transfer, draft sharing. Import path runs `validateImportSchema` — top-level key whitelist + per-key type-check + recursive sanitize of nested containers (fields/parties/routing/string-arrays) + strip-unknown + confirm-before-replace — so `restoreFullState` only ever sees a sanitized object.
...
(Both 2026-04-25 brainstorm proposals approved and now shipped — see Approved (Remaining) section above.)

- **Draft Compare / Diff** (proposed 2026-06-26) — "compare" button opens a drawer where the user picks two saved drafts (or current-form vs a saved draft) and sees a field-by-field comparison: From, To, Subj, SSIC, Date, body, via/refs/encls. Differing fields highlighted; identical fields collapsed. Read-only — never mutates either draft. Useful for version review and for confirming a Reply draft against the original. Pure-DOM render, localStorage-backed, mutual-exclusion with existing drawers. Fits the utility-flagship identity (reliability/correctness, no novelty toys).
- **Subject Line Helper** (proposed 2026-06-26) — inline assist on the Subject field reflecting naval convention: one-click "UPPERCASE" button (naval subjects are conventionally all-caps), a live character counter with the existing ≤80-char advisory shown in-form (not just in the Quality Checker), and trailing-punctuation strip. Advisory + opt-in transform only — never auto-rewrites on keystroke, never blocks. Small, in-identity correctness win that complements the Letter Quality Checker.
...
| 2026-06-26 | Tock | Approved queue now empty after shipping the two features above. Brainstormed 2 PENDING proposals (no council): Draft Compare / Diff, Subject Line Helper. |
| 2026-06-26 | Tock | Letter Quality Checker + Portable Draft Export/Import SHIPPED. Both were implemented in index.html and had passed PLAN + IMPLEMENTATION (impl UI mutual-exclusion OBJECT was a false positive, resolved by John 2026-05-28 — see COUNCIL_ESCALATION.md). This session resumed the build: pre-filter PASS (test_project PASS, security_scan CLEAN, eval_bugs pre-existing-only — new feature region L4453+ clean), then TESTS 7/7 APPROVE and OUTCOME 7/7 APPROVE. Quality Checker: 7 advisory naval-format checks, pure-DOM render. Export/Import: whitelist + recursive schema validation in import path. Both approved roadmap items now complete; remaining-approved list is empty. |
