...
**No goalpost-moving (added 2026-04-09, broadened 2026-04-09 to cross-gate):** When an angle's objection on a feature has been resolved or overruled by the project owner, the SAME angle may NOT raise a NEW objection on the SAME feature at ANY later gate (not just the same gate) unless the new objection cites NEW evidence not present in the original review. This prevents infinite-objection loops where an angle simply rephrases its objection after being overruled — including the subtler case where an angle objects on the PLAN gate, gets overruled, then raises a similar-but-reframed objection on the IMPLEMENTATION gate (different gate, same underlying concern, no new evidence). **Cross-gate tracking:** The orchestrator must track per `(angle, project, feature)` the set of underlying concerns raised across ALL gates, not just within a single gate. When evaluating a new OBJECT from an angle that has previously objected on the same `(project, feature)` at any gate, compare the new `reason` against all `prior_reason` values for that tuple. If keyword overlap exceeds 0.6 (count shared non-stopword tokens / max token count of the two reasons), auto-downgrade the new objection to advisory. The 0.6 threshold is deliberately conservative — a false negative (letting a goalpost move through) is caught by human escalation review, while a false positive (suppressing a legitimate new concern) would erode trust. **Same-gate check (original rule):** if attempt N's `council_<gate>.json` for an angle was OBJECT and attempt N+1 from the same angle is also OBJECT, the orchestrator must compare the two `reason` fields. If they describe the same underlying concern (even with different framing), the second objection is auto-downgraded to advisory. **Concrete examples:** (1) Same-gate: SECURITY first objected that localStorage entries needed encryption; after that was resolved with a user-warning, SECURITY pivoted to attacking the host CSP architecture as "critical XSS" — same angle, same feature, completely different reason, no new evidence. That pivot is a goalpost move. (2) Cross-gate: SECURITY objects on the PLAN gate about plan.md being "unsanitized AI output" (medium, overruled). If SECURITY then objects on the IMPLEMENTATION gate that "the generated code from plan.md wasn't sanitized" with no new evidence, keyword overlap between the two reasons would exceed 0.6 and the objection is auto-downgraded.

**No host-architecture objections in per-feature reviews (added 2026-04-09):** Council angles reviewing a single feature may NOT raise objections targeting the project's host architecture (CSP policy, framework choice, single-file vs multi-file, storage layer, etc.). Those decisions are made once at the project level and recorded in `<project>/CONSTRAINTS.md`. If a feature genuinely introduces a new attack surface or architectural concern, the angle must cite the specific code location where the new surface is introduced. "The whole app is vulnerable because of X" is not a per-feature objection; it is an architecture critique and must be raised through a separate `architecture_review_request` field, not as a per-feature OBJECT verdict. Council orchestrators must check whether `<project>/CONSTRAINTS.md` exists and whether the objection's target falls within its declared constraints; if so, the objection is auto-downgraded to advisory.

**Plan artifacts are human-review documents (added 2026-04-09 after adopt-planning-mode escalation):** Files produced by the PLAN gate (`plan.md`, `council_plan.json`, `COUNCIL_ESCALATION.md`) are human-review artifacts. No downstream automated parser consumes file paths, function names, or other structured content within them. SECURITY may NOT require producer-side sanitization of plan artifact content (e.g., "sanitize file paths in plan.md to prevent path traversal") absent a concrete downstream parser that reads and acts on those values. If a future system is introduced that programmatically consumes plan artifacts, sanitization is the CONSUMER's responsibility — it must be added at the parser before that consumer ships, not retroactively imposed on the producer. This rule exists because SECURITY raised a hypothetical-future-system objection on adopt-planning-mode (2026-04-08, plan gate attempt 2, medium severity) claiming plan.md is "unsanitized AI output" that could contain XSS payloads — but plan.md has no render surface and no automated consumer. 6 of 7 angles approved; SECURITY was overridden.
...
**Council enforcement rules are now LIVE in code (added 2026-04-22, expanded to 4 rules 2026-04-25):** As of commits `967c98e` (initial 3 rules) and `5c75dd5` (rule 4 added), `council.py` implements four enforcement rules that previously existed only in learnings.md prose:
...
4. **BUGS hallucination auto-downgrade (added 2026-04-25)** — `detect_bugs_hallucination` auto-downgrades BUGS OBJECTs whose `reason` uses "undefined/undeclared/missing/not defined/does not exist" phrasing AND cites backtick-quoted symbol names that **are actually defined** in cited files. Two-layer check: (a) cited-line window check using `file:line` references in `evidence`, (b) language-aware definition-pattern check across the whole file (JS/TS: `function X`, `const/let/var X =`, line-anchored method shorthand `X(...) {`; Python: `def/class X`, line-anchored `X =`). Symbols are extracted from the prose **before** each claim keyword (not after) so suggested replacements like "use `setDraftStatus` instead" don't trigger false matches. Closed off the LST rounds 7-9 + Template Library rounds 1-3 hallucination loops. See `experiments/fix-council-bugs-hallucination/` for the tick.
...
### falling-sand (2026-04-02)
- **KEEP**: `Uint8Array grid + Uint8Array meta + Uint8Array updated` — three parallel arrays for cellular automaton. Grid holds material type (0–11), meta holds variation+life, updated prevents double-processing in same frame pass. Minimal memory, cache-friendly layout.
- **KEEP**: Bottom-to-top scan (`for y = GH-1 downTo 0`) + alternating L-to-R/R-to-L (`tick & 1`) — prevents sand/water from drifting in one direction and allows gravity to work correctly in a single pass.
- **KEEP**: `mark(destination)` after `moveTo()` — essential for upward-moving particles (smoke, steam, fire). Since we scan bottom-to-top, upward destinations haven't been visited yet; marking prevents double-processing after the particle moves up.
- **KEEP**: `Uint8Array` life decrement pattern: check `meta[i] === 0` before decrement, then `meta[i]--`, then check `=== 0` again. This prevents Uint8 wrap-around (1→0→255 never happens because we guard the 0 case first).
- **KEEP**: `image-rendering: pixelated` + canvas buffer smaller than CSS size — chunky pixel aesthetic at any screen size with no upscaling blur. Internal resolution ~200×140, SCALE computed dynamically to fill available area.
- **KEEP**: Stretch canvas CSS to fill exact available area (`canvas.style.width = aw + 'px'`) rather than `GW * SCALE + 'px'` — prevents visible gap when GW*SCALE < aw due to floor rounding.
- **KEEP**: Module-level `DIRS4` and `DIRS8` constants — never allocate direction arrays inside simulation functions. Cellular automaton functions run 60fps × thousands of cells; per-call array creation causes GC stutter.
- **KEEP**: `canvas.toBlob()` + `URL.createObjectURL` + DOM append/click/remove + `setTimeout(revokeObjectURL, 5000)` — the standard pattern for PNG save. Works in both Chrome and Firefox (Firefox requires DOM append before `.click()`).
- **IMPROVE**: Oil color (RGB 26,18,8) was nearly invisible against #0a0a0a background. Any dark material needs at least 40+ luminance to be distinguishable. Check all near-black materials against page background during design.
- **INSIGHT**: For materials that "float on liquid" (oil on water): the simulation rule is simple — if oil's below-cell is water, swap them. This naturally creates the floating behavior across the whole body.
...
### epicycle — Fourier Epicycles Drawing (2026-04-02)
- **KEEP**: Complex DFT formula: `re = Σ(px*cos + py*sin)/N`, `im = Σ(py*cos - px*sin)/N`. This correctly handles 2D points as complex numbers z_n = x_n + i*y_n, enabling reconstruction via rotating arms. Centroid subtraction before DFT ensures the DC term vanishes and the origin becomes the centroid.
- **KEEP**: Sort DFT coefficients by amplitude descending — biggest circles first. This is both correct (most energy first) and visually satisfying (see approximation improve as you add circles). The circle slider then maps 1..N to this sorted order.
- **KEEP**: `getPositions(t)` pattern — accumulate x/y by adding each epicycle's contribution (`amplitude * cos/sin(2π*freq*t/N + phase)`), returning the full chain of positions. Both the center of each ring AND the tip are naturally available from consecutive entries.
- **KEEP**: Trace wrapping strategy — when `playT >= N`, reset `tracePoints = []` and subtract N. This produces a clean cycle-restart rather than an ever-growing line. Combined with the quality badge showing "exact reconstruction" or "N% of signal", users understand what they're seeing.
- **KEEP**: Preset parametric paths (heart, lemniscate, rhodonea rose, Tschirnhausen cubic) as unit-scale coordinates scaled to `Math.min(W, H) * 0.42`. Scale by max radius so shapes fill ~84% of the shorter dimension regardless of screen size.
- **KEEP**: `canvas.toBlob()` + `URL.createObjectURL` + DOM append/click/remove + `setTimeout(revokeObjectURL, 5000)` for PNG export. Firefox requires DOM append before `.click()`. Always render a fresh frame before calling toBlob to ensure current state is captured.
- **KEEP**: `stopAnim()` (cancel rAF) when pausing — don't spin rAF doing nothing for potentially minutes. Resume by restarting rAF with fresh `lastTime = performance.now()`. The animate loop already caps dt at 0.05s on resume as additional safety.
- **IMPROVE (council BUGS)**: `renderPlayback` had `var N = dftCoeffs.length` declared but never used — remove unused variables before shipping.
- **IMPROVE (council UI)**: Preset buttons at 28px height; main buttons at 32px — both below 44px Apple HIG. Kept at these values to preserve dense toolbar layout on mobile, but 36px should be the practical floor.
- **IMPROVE (council USEFULNESS)**: Quality badge ("exact reconstruction" / "N% of signal") turns the abstract circles slider into an understandable control. Always explain what a slider does through a live readout, not just its label.
- **IMPROVE (council GUIDE)**: Empty `#legend` div as dead code confuses the next maintainer. If a planned feature isn't implemented, remove its stub entirely.
- **INSIGHT**: For any parametric curve visualization, the DFT + epicycle trick works for ANY closed path — the math doesn't care about the source. User-drawn strokes and mathematical presets go through the same pipeline. The only difference is how the input points are generated.
- **INSIGHT**: The "quality" framing (percentage of signal) is more intuitive than "number of circles" for the slider. Users understand "50% quality" more readily than "25 out of 50 circles". Show both: the count for precision, the percentage for meaning.
- **INSIGHT**: When `state === 'computing'`, a 30ms timeout deferral allows the DOM to repaint ("computing DFT...") before the O(N²) loop blocks the thread. This is the correct pattern for heavy synchronous work in a browser — not workers, not async, just one setTimeout to let the repaint happen first.
- **TEST CAUGHT**: Unused `var N` in renderPlayback — caught by council review, not static tests. Council review's BUGS angle is the only check that catches unused variables.

### field-lines — Magnetic Field Visualizer (2026-04-02)
- **KEEP**: RK4 streamline tracing on unit-normalized field — `rk4Step(x, y, stepSize)` normalizes k1–k4 to unit vectors before combining. Produces smooth, well-spaced field lines. Step size of ±4px is the sweet spot for canvas-scale field lines (large enough to run fast, small enough to not miss curves).
- **KEEP**: Superposition via simple additive field evaluation — `fieldAt(x, y)` loops over all sources and sums contributions. O(sources) per evaluation; totally fine for <20 sources × 100k evaluations/recompute.
- **KEEP**: `cachedLines`/`cachedFilings`/`densityImg` invalidation pattern — set all to null on source change, recompute inside `render()` on first dirty frame. Prevents redundant O(N) recomputes every rAF while still updating immediately after user interaction.
- **KEEP**: Iron filings as oriented dashes — `Math.atan2(by, bx)` gives angle; draw dash from `(x-cos*len, y-sin*len)` to `(x+cos*len, y+sin*len)`. Length 7px + alpha scaled to field magnitude creates beautiful tactile feel.
- **KEEP**: Spectral density heat map — map t ∈ [0,1] to black→blue→cyan→white in three linear segments. More visually dramatic and informative than single-color gradient. Uint32Array write to ImageData buffer for performance.
- **KEEP**: Preset configurations with `loadPreset(name)` — complete state reset, then programmatic source placement using `Math.min(W, H) * 0.2` as gap for responsive layout. Dipole, quadrupole, 2-wire, solenoid. Presets drive immediate wow-factor on page load.
- **KEEP**: `!didDrag && dragging === null` guard before adding source on tap-up — prevents accidental source addition when user is dragging an existing source. Track `didDrag` via `dx*dx + dy*dy > 16` threshold.
- **BUG (self-audit)**: `var nLines = 16` declared inside `computeFieldLines()` but never read — the actual line count was set below as `count`. Dead variable. Removed.
- **IMPROVE (council UI)**: Button `min-height: 28px` is below the 36px practical floor. Always use 36px as the minimum for any clickable control. 44px for primary CTAs.
- **IMPROVE (council USEFULNESS)**: Preset configurations are mandatory for physics visualizers. Users need an immediate "wow" state before they understand how to build their own. Always ship with 3–4 memorable presets.
- **IMPROVE (council COOL)**: Single-color density gradient (dark→cyan) looks flat. Spectral color mapping (black→blue→cyan→white) adds dramatic depth and encodes more information visually.
- **INSIGHT**: For any field visualization, the three rendering modes (streamlines, vector field samples, scalar magnitude heat map) are a reusable trio. Each shows something different: topology (lines), direction (filings), strength (density). Together they tell the complete field story.
...
### penrose — Penrose P3 Tiling Explorer (2026-04-02)
- **KEEP**: Robinson triangle subdivision algorithm for P3 Penrose tiling: `acute (A) → 1A + 1B`, `obtuse (B) → 1A + 2B`. Produces valid aperiodic tiling with fat/thin ratio → φ. Verified geometrically with actual coordinates.
- **KEEP**: Batching Canvas 2D fill paths by type — two `beginPath()...fill()` calls for all acute fills, then all obtuse fills, then one stroke path for all edges. Much faster than per-triangle fills. For 16000 triangles, this stays smooth.
- **KEEP**: Pan/zoom state as `(panX, panY, zoom)` where `screenX = panX + worldX * zoom`. Zoom around mouse: `worldX = (mouseX - panX) / zoom; newPanX = mouseX - worldX * newZoom`. This formula works identically for wheel zoom, pinch zoom, and button zoom.
- **KEEP**: Pinch-zoom tracking both zoom AND pan simultaneously: store the world point under the INITIAL pinch midpoint, then keep that world point under the CURRENT midpoint as fingers move. `panX = mid.x - worldX * newZoom` where `mid` is current finger midpoint. This is the correct pattern.
- **KEEP**: `history = []` stack of `{tris, gen}` for deflate — simple undo without recomputing. Push on inflate, pop on deflate. Only store tris array (not pan/zoom — viewport stays stable through deflate).
- **KEEP**: Initial auto-inflate to gen 5 (890 tiles) — starts visually dense enough to be interesting without being slow. Always auto-advance simulation tools to a "interesting" starting state.
- **KEEP**: Robinson triangle derivation from first principles using coordinate verification — place triangle at origin, compute all intermediate points, verify type by angle calculation. Saved hours of debugging vs. trusting abstract rules.
- **IMPROVE**: Button min-height must be 36px minimum (caught by UI review, was 32px). 36px is the practical floor for mobile tap targets based on repeated learnings.
- **INSIGHT**: For Penrose P3, the subdivision scale changes by 1/φ per step: `A_type: legs=φ/φ^n, base=1/φ^n`. All new triangles from one step share the same scale. Area conservation: `1·area(A,1/φ) + 1·area(B,1/φ) = area(A,1)` and `1·area(A,1/φ) + 2·area(B,1/φ) = area(B,1)`. Both hold due to φ² = φ+1.
- **INSIGHT**: Robinson triangle vertex convention matters: in `inflate()`, the FIRST vertex (A) is always the apex. Chirality (left/right handedness) determines whether the new acute gets `{A:C, B:P, C:B}` or the flipped version. Getting this wrong produces a "pinwheel" instead of a proper Penrose tiling.
- **INSIGHT**: For quasi-periodic tilings, the fat/thin rhombus ratio φ only emerges after many subdivisions. At gen 0 (10 pure acute), ratio = ∞. By gen 5 (340A/550B), ratio B/A = 1.62 ≈ φ. The ratio converges from above.
- **TEST CAUGHT**: Double-tap inflate registered as SECOND `addEventListener('pointerup', ...)` fires AFTER the first pointer-tracking listener. No conflict — each handler is independent. Both fire in registration order. This pattern is safe.

### css-units (2026-04-03)
- **KEEP**: Single-state-object + one `render()` function pattern — `state = { value, unit, baseFontSize, parentFontSize, viewportW, viewportH }` with one `render()` that rebuilds everything from scratch. No sync bugs possible. For a pure conversion tool with no animation, full rebuild on every change is totally fine.
- **KEEP**: Separate `toPx(value, unit, ctx)` + `fromPx(px, unit, ctx)` as pure functions — enables easy testing and makes conversion logic completely decoupled from DOM. The two-function pattern is the right way to handle any bidirectional unit system.
- **KEEP**: `formatVal(v)` adaptive precision — `abs >= 1000: .toFixed(2)`, `abs >= 10: .toFixed(4)`, `abs >= 0.01: .toFixed(5)`, else `toExponential(3)`. Automatically gives more decimals for small values and fewer for large ones.
- **KEEP**: Device preset buttons (iPhone SE/14, iPad, Laptop, Desktop, 4K) for viewport dimensions — transforms an abstract "Viewport W/H" input into something immediately actionable. Devs think in device names, not pixel counts.
- **KEEP**: `localStorage` for context settings (`baseFontSize`, `parentFontSize`, `viewportW`, `viewportH`) — a conversion tool you configured once should stay configured. The persisted state is load-on-init / save-on-change pattern; no throttle needed since it's low frequency.
- **KEEP**: `rulerTrack` DOM ref cached at module load level — querying `getElementById` inside per-render functions is wasteful. Always cache refs once.
- **IMPROVE**: Unit pill `min-height: 28px` violated the 36px floor — caught by self-audit. This is the **fourth time** this mistake appeared. Must make it a mental default: every interactive element is 36px minimum. No exceptions.
- **IMPROVE**: `document.getElementById('ruler-track')` was called inside `updateRuler()` on every render instead of caching at init. Caught by self-audit. Cache ALL DOM refs at module start.
- **INSIGHT**: The "bookmark test" is the right filter for utility tools: "would a developer open this tab multiple times per week instead of googling?" CSS unit conversion passes — devs hit it when converting design specs, debugging em inheritance, setting viewport-relative sizes. Tools that replace a Google search + mental math are high-value.
...
### cron-builder (2026-04-03)
- **KEEP**: `expandField(str, lo, hi)` as the core cron parsing primitive — handles `*`, `*/n`, `a-b`, `a-b/n`, `a,b,c` and all combinations. Split on `,` first, then detect `/`, then `-`, then literal. Returns a sorted deduplicated array. Cleanly composable for any range-based config syntax.
- **KEEP**: `parseCron` post-processing for DOW Sunday=7 alias — expand with range 0-7, then normalize: `dv = rawDows[di] === 7 ? 0 : rawDows[di]`, deduplicate, sort. Handles `0 0 * * 7` (valid in most cron implementations) gracefully without a special case in the parser.
- **KEEP**: `copyTimeoutId = null` pattern — track the timeout ID for "Copied!" label so rapid clicks cancel the previous timer. `if (copyTimeoutId !== null) clearTimeout(copyTimeoutId)` before setting new one. Applies to any "transient state" UI label (toasts, button feedback).
- **KEEP**: `toTZParts(d, tz)` via `Intl.DateTimeFormat.formatToParts()` — correctly extracts {min, hour, dom, month, dow} in any IANA timezone. The `hour12: false` flag plus `% 24` handles midnight (returns "24" in some environments). Wrap in try/catch to handle invalid timezone strings.
- **KEEP**: Next-run iteration at 1-minute granularity is totally fine — `maxIter = 500000` covers ~347 days. For any cron that matches at least once per year this terminates well within limits. Real-world crons run far more often.
- **KEEP**: `buildDescription` separates raw field string checks (`isWildcard(raw)`, `isStep(raw)`) from expanded value arrays — description needs to distinguish `*` from `0-23` (both expand to all hours, but different semantics for humans). Always keep the raw string alongside the expanded set.
- **IMPROVE**: `eval_bugs.py [classname-overwrite]` flags `el.className = 'some-class'` on freshly-created elements (li, span, button) as false positives. These are safe. Only flag if the element has pre-existing classes. Self-audit: read the actual line, not just the pattern message.
- **INSIGHT**: Cron's DOM/DOW ambiguity rule: if BOTH day-of-month and day-of-week are restricted (not `*`), most cron implementations use OR semantics. If only one is restricted, use that one. Always implement this correctly — `0 0 15 * 1` fires on the 15th OR any Monday, not just Mondays on the 15th.
- **INSIGHT**: `Intl.DateTimeFormat().resolvedOptions().timeZone` auto-detects local timezone — try/catch it and check against the known list. If present, pre-select it. Zero-config UX: the tool works correctly for the user's timezone without any action.
- **INSIGHT**: For any scheduling tool, "next 10 run times" with relative times ("in 2h", "in 3d") is more useful than absolute times alone. Absolute tells you when; relative tells you how soon you'll need to wait. Show both.
- **TEST CAUGHT**: Sunday=7 alias bug found during council review BUGS angle. `"0 0 * * 7"` (Sunday midnight, valid in cron) threw "Value 7 out of range [0,6]". Fixed by expanding dow 0-7 then normalizing 7→0.

### json-diff (2026-04-03)
- **KEEP**: `Object.create(null)` for accumulator objects used as key sets — creates a truly prototype-free object, immune to prototype pollution. Combine with `Object.keys()` iteration instead of `for...in` for safe object key enumeration.
- **KEEP**: Per-button closure `var tid = null` for copy feedback timeout — each copy button tracks its own timeout independently. Shared module-level `copyTimeoutId` causes rapid clicks on different buttons to cancel each other. Always scope transient UI state to the element it belongs to.
- **KEEP**: Recursive diff engine pattern: `diffValues(key, left, right)` dispatches by type pair — both objects → `diffObjects`, both arrays → `diffArrays`, else scalar comparison. Returns typed `DiffNode` (`added`|`removed`|`changed`|`unchanged`|`container`). Clean separation between diffing and rendering.
- **KEEP**: `hasChanges(nodes)` recursive predicate — used to set the gutter symbol on container nodes (`~` if subtree has any changes, space if all unchanged). Separates "does this subtree have changes?" from rendering logic.
- **KEEP**: `countChanges(nodes, acc)` accumulator pattern with optional `acc` param — `if (!acc) acc = { added:0, ... }` lets it serve as both a top-level call and recursive call without a wrapper. Clean and allocation-free.
- **KEEP**: `@media (hover: none)` to show copy buttons on touch devices — devices without hover capability never see the hover-triggered opacity. `opacity: 0.5` fallback ensures the button is always discoverable on mobile.
- **IMPROVE**: `/"/g` regex literal again caused test brace-balance failure (same as json-ts). This is the SECOND time this happened. Lesson now fully internalized: any regex literal containing `"` must use `\x22`. Add this to pre-build checklist.
- **DISCARD**: Module-level `copyTimeoutId` shared across all copy buttons — causes cancel-on-different-button bug. Always use closure-scoped timeout IDs.
- **INSIGHT**: For structural diff tools, the "Show Unchanged" toggle is the single most important UX affordance. Without it, a small diff in a large object is buried in noise. Default to hidden unchanged; let users expand when they need context.
- **INSIGHT**: `diffObjects` key enumeration: build a `Object.create(null)` map of all keys from both sides, then `Object.keys()` that map. This is correct, prototype-safe, and preserves insertion order (left-first, then right additions). Never use `for...in` without `hasOwnProperty` guard on untrusted objects.
- **TEST CAUGHT**: `/"/g` regex → test brace-balance false failure. Fix: `\x22`. This is now a permanent pattern in my pre-build mental checklist.
### regex-lab (2026-04-03)
- **KEEP**: Flat row-list with `depth` field for rendering explanation trees — instead of building an actual tree and recursing for rendering, push each token as a row with `depth: N` during tokenizer traversal. Rendering becomes a simple `for` loop with CSS indent classes. Zero recursive rendering, easy to debug.
- **KEEP**: Recursive descent parser for regex tokenization — `parseAlternation → parseBranch → parseAtom → parseGroup/Class/Escape` with `parseQuantifier` applied to each atom result. The key is the flat output (push rows) rather than building a nested AST — this simplifies rendering enormously.
- **KEEP**: Preset objects with `pattern`, `flags`, and `test` fields — loading a preset sets all three at once and immediately shows a working example. Instant value, no interaction needed.
- **BUG (test)**: `//` in JS string test values causes the test's comment stripper (`//.*?$`) to delete everything from `//` to end-of-line, breaking string boundaries and cascading into false brace-balance failures. The existing rule "split http:// as 'http:/' + '/'" must be applied to ALL string values, including test strings — not just URL patterns.
- **IMPROVE**: `document.querySelectorAll('.flag-btn')` was called in BOTH `init()` and `loadPreset()`. Fixed by caching the NodeList in a module-level `elAllFlagBtns` var at init. All DOM queries must be cached at init — applies to `querySelectorAll` too, not just `getElementById`.
- **INSIGHT**: The test brace-balance checker strips strings BEFORE counting braces. If a `//` in a string value causes that string to be unterminated (comment stripper runs FIRST, before string stripping), the remaining `'` creates a cascading misparse: adjacent code is consumed as "string content", and dozens of later strings are missed. The cascade is hard to detect without tracing `re.finditer` output. Prevention: NO `//` anywhere in any JS string literal.
...
### unix-ts (2026-04-05)
- **KEEP**: `valElMap` — a flat object caching all value, copy-button, and reverse-output elements by key at `init()` time. Both `setValue(key, text)` and `copyRow(key)` look up `valElMap[key]` instead of calling `getElementById`. Eliminates the `[dom-query-in-render]` eval_bugs flag and is the cleanest pattern for tools with many parallel output rows.
- **KEEP**: `Math.abs(n) > 1e12` as the auto-detect threshold for ms vs s. Any timestamp after year 33658 is 13+ digits in seconds — safe cutoff. Works for negative timestamps (past Epoch). No extra input from the user.
- **KEEP**: UTC parse for `datetime-local` input: detect colon count in time part, append `:00Z` if <2 colons (no seconds), or just `Z` if already has seconds. Avoids dead-code paths and is forward-compatible with browsers that omit seconds from datetime-local value.
- **KEEP**: `Intl.DateTimeFormat().resolvedOptions().timeZone` in a try/catch to get the local IANA timezone name (e.g. "America/New_York"). Wrap in try/catch, fall back to "(local)" if unavailable. Transforms a generic "local" label into something actually useful.
- **KEEP**: `clearInterval(relTimerId)` guard before `relTimerId = setInterval(...)` — even when init is called once, the guard is a zero-cost safety net that addresses [stale-timeout-on-reset] semantically.
- **IMPROVE**: Copy buttons should be 44×44 minimum. Not 36×44. This is the FIFTH time this specific mistake appeared (see jwt-inspector, cron-builder, css-units). Adding to pre-build mental checklist: ALL interactive elements are 44px × 44px. Touch targets include width, not just height.
...
### url-dissect (2026-04-06)
- **KEEP**: `URL()` API for zero-dep URL parsing — `u.protocol`, `u.port`, `u.pathname`, `u.search`, `u.hash`, `u.origin`. Combine with raw `u.search.slice(1).split('&')` to get raw encoded values that `URLSearchParams` auto-decodes away. This gives both raw and decoded values for the encoding audit.
- **KEEP**: `DEFAULT_PORTS` lookup table for protocol-default-port awareness — show "443 — default for https" when `u.port === ''`. Flag non-standard explicitly.
- **KEEP**: Resolve tab via `new URL(relative, base)` — one-liner that handles `../` traversal, scheme-relative, and absolute refs correctly. Zero math needed.
- **KEEP**: Bulk URL extraction with `new RegExp('https?:\\/\\/[^\\s<>]+', 'g')` + trim trailing punctuation by `charCodeAt` (41=paren, 93=bracket, 125=brace) — avoids putting `)`, `}` in string or regex character classes that confuse the brace counter.
- **IMPROVE**: test_project.py brace/paren/bracket balance checker has three known quirks to watch: (1) `//` in any string literal gets eaten by the comment stripper first, leaving an unclosed string; (2) `'` inside a regex literal (like `/construct 'URL':/`) can cause the single-quote stripper to consume multi-line code; (3) `{n}` quantifiers in regex literals count as unbalanced braces. Fix all three by using `new RegExp('...')` with string args and splitting `//` as `'/' + '/'`.
- **INSIGHT**: The triple interaction of comment strip → string strip → brace count in test_project.py means ANY `//` in a single-quoted string (e.g. `'https://'`) will corrupt the brace balance check. Safest rule: never put `//` inside a quoted string. Use `'https:' + '/' + '/'` or `String.fromCharCode(47,47)`.
- **INSIGHT**: Regex literals with single quotes inside (e.g. `/construct 'URL':/i`) are invisible to the string stripper. If the surrounding line also has real strings, the unmatched `'` inside the regex can cause the stripper to consume code across many lines. Replace with `new RegExp('...', 'i')` to keep the pattern in a properly-stripped string.
- **COUNCIL**: Gemini MCP unavailable — PARTIAL REVIEW logged. Internal 6-angle review: no bugs found. Security: uses `execCommand` as clipboard fallback only (standard pattern). Performance: IP_RE hoisted to module level. UX: encoding issue badges on query params are the core differentiating feature.

## Tock: markdown-deck — Per-slide talk time budget (2026-04-06)
- **KEEP**: SVG `stroke-dashoffset` drain pattern for countdown rings: `circumference = 2π×r`, `dashoffset = (elapsed/budget) × circumference`. With `transform="rotate(-90 cx cy)"` on the arc circle, drain starts at 12 o'clock. Smooth with `transition: stroke-dashoffset 0.9s linear`.
- **KEEP**: Web Audio API two-tone chime with staggered oscillators — `tone(880, 0, 0.35); tone(660, 0.2, 0.45)` gives a pleasant "ding-dong" with zero deps. Use `linearRampToValueAtTime` for attack and `exponentialRampToValueAtTime` for release to avoid clicks.
- **KEEP**: `_chimeFired` boolean guard per-slide prevents the chime from firing every second once overtime. Reset it in `startSlideTimer()` when a new slide begins.
- **KEEP**: Lazy `AudioContext` init via `_getAudioCtx()` — defer creation until first chime to avoid "AudioContext not started" browser warnings on page load.
...
### [naval-scribe: Command Address Book] (2026-04-08)
- **KEEP**: For tock features that are drawer-pattern extensions, open addr_drawer with mutual-exclusion close of all other drawers — add `addrDrawer.classList.remove('open')` to existing draftsBtn/importBtn/chainBtn listeners with `true` (capture phase) to keep open/close logic centralized.
- **KEEP**: `saveAddr(arr)` returns `boolean` — caller must check the return value before committing the in-memory array update. Pattern: `if (saveAddr(next)) { renderAddr(); clearInputs(); }` — never update state on false.
- **KEEP**: `document.createDocumentFragment()` for addr-list batch render — even though typical address book is 0-50 entries (below the >50 threshold), the pattern was used because the full re-render on every add/delete rebuilds the entire list.
- **KEEP**: All action buttons in renderAddr() use `.textContent` (not innerHTML) for user-controlled entry.name and entry.label — critical for XSS safety when values come from user-entered localStorage.
- **IMPROVE**: Static HTML buttons that have matching JS-generated siblings should get aria-labels at the HTML layer (not just the JS layer) — `#addr-save-btn` was initially missing its aria-label until TESTS gate caught it. Always audit HTML-layer buttons alongside JS-generated ones.
- **INSIGHT**: File-truncation false positives are systematic when --context is a 3370-line HTML file — BUGS/SECURITY angles consistently object to "missing implementation" when the JS is past ~line 1000. Fix: always append the relevant JS block in --inline for large flagship files. Tests gate attempt 1 and Outcome gate attempt 1 both wasted a round trip due to truncation.
- **INSIGHT**: SECURITY angle false-positive pattern: "innerHTML with user input" raised for SSIC autocomplete — but SSIC_CODES is a hardcoded constant array, not user input. The eval_bugs.py [innerhtml-xss] pattern fires on ANY innerHTML, not just user-controlled innerHTML. Distinction must be made explicit in inline context.
- **COUNCIL**: PLAN(attempt 2, 5/7 + structural override on SECURITY/COOL) | IMPLEMENTATION(attempt 2 after John fix: font-size 0.48rem→0.55rem, aria-labels) | TESTS(attempt 2, 7/7 after providing addr-book JS block in inline) | OUTCOME(attempt 2, 7/7 after providing addr-book JS block + explicit textContent/try-catch evidence)

### [adopt-planning-mode: infrastructure tick] (2026-04-12)
- **KEEP**: For infrastructure council calls, inject `resume_instructions` + `council_focus` from queue item into every `--inline` payload. Without this, council angles re-raise objections that have already been resolved by the human.
- **INSIGHT**: COOL angle criteria (cool.md) are scoped to user-facing HTML tools — "URL memorability", "worth telling a friend", "signature move". These criteria do NOT apply to internal pipeline prompt or config changes. When COOL objects on infrastructure, add a note to attempt 2's inline clarifying scope.
- **INSIGHT**: Infrastructure OUTCOME gate: COOL "lacks signature move" (critical severity) → 6/7 approved on attempt 1 → 7/7 on attempt 2 after injecting explanation that COOL's user-facing criteria are inapplicable to a .github/workflows/.md change.
- **COUNCIL**: PLAN(7/7 approve, after 2 prior escalations resolved by John) | IMPLEMENTATION(7/7 approve, GUIDE objection resolved by human — rephrased 'structured planning mode' to 'using the following structured format') | TESTS(7/7 approve) | OUTCOME(attempt 2, 7/7 approve after COOL inapplicability context)

### [markdown-deck: Named Snapshots] (2026-04-13)
- **KEEP**: `_saveSnaps(arr)` returns `boolean` — `takeSnapshot()` only updates `_snaps` in-memory if persistence succeeds. Pattern: `if (!_saveSnaps(candidate)) { showToast('Storage full'); return; }` — never commit phantom state on failure.
- **KEEP**: Toast pill pattern for transient save confirmations — show count in the message ("Snapshot saved (N/20)") so user knows the current checkpoint count without opening the modal.
- **KEEP**: `contentEditable` label with revert-on-empty on blur — `if (!newLabel) { newLabel = origLabel; labelEl.textContent = origLabel; }`. Prevents orphaned nameless snapshots.
- **KEEP**: `document.createDocumentFragment()` for modal list rendering — even for bounded lists (max 20), the fragment-based batch build is the pattern throughout this flagship; keep consistent.
- **KEEP**: ID generation `Date.now().toString(36) + Math.random().toString(36).slice(2, 8)` — collision-resistant without external libs; suitable for in-session bounded lists (≤20 items).
- **INSIGHT**: Pre-filter `brace_balance` false positive fires on JavaScript regex literals containing `{}` characters (e.g., `/\{([^}]*)\}/g`). This failure was pre-existing in the flagship code before the Named Snapshots addition. When test_project.py brace_balance fails, check whether all failures trace to regex patterns rather than actual unbalanced code braces.
- **INSIGHT**: All 84 eval_bugs.py and all 11 security_scan.py findings were pre-existing in the flagship at HEAD. Pre-filter gate "all pre-existing" classification: when confirmed via `git show HEAD:<file>`, document and proceed — council tests gate reviews completeness, not pre-filter raw counts.
- **COUNCIL**: PLAN(7/7 approve) | IMPLEMENTATION(6/7 — UI objection on font sizes resolved by human: 0.55rem + 6px/12px padding + min-height 28px) | TESTS(7/7 approve) | OUTCOME(7/7 approve)

### [infra-guardrails: infrastructure tick] (2026-04-21)
- **KEEP**: `[^|]*` (not `[^|]+`) in ROW_RE for markdown pipe-table parsing — empty fields must match the regex so the subsequent `if not all(fields)` check can produce specific "Row CN: one or more empty fields" errors instead of silently skipping the row and producing a misleading "Missing constraint: CN" error. Always use `[^|]*` when you want to detect-and-report empty fields.
- **KEEP**: Scope pipe-table row extraction to the target section only using a SECTION_RE slice — prevents rows in other `## ` sections with the same format from polluting the found-IDs set.
...
### [infra-memory-feedback: infrastructure tick] (2026-04-23)
- **KEEP**: `json.dumps([field1, field2, ...])` for content-hash input — when building a stable ID from multiple string fields, JSON serialization is collision-proof (escapes embedded characters including the delimiter itself). Do NOT use `"|".join(fields)` — pipe characters in the data fields produce identical concatenations for distinct records. The progression of hash bugs in this tick (SHA1 → SHA256 → no-truncation → json.dumps) shows the delimiter-encoding issue is the final, non-obvious failure mode.
- **KEEP**: `schema_versions` table for hash-algo migration — when the content-hash algorithm changes, stale rows must be invalidated and regenerated (not compared against the new algo). A `(key TEXT, value INTEGER)` KV table with `algo_version` entry enables `DELETE WHERE escalation_id IS NOT NULL` followed by idempotent re-backfill. Manual rows (`escalation_id IS NULL`) are preserved across migrations.
- **KEEP**: Descriptive command aliases without renaming — when UI raises a late-stage naming concern, add `elif cmd in ('old-name', 'new-descriptive-name')` as a 2-line fix. Old callers work unchanged; new users find the intuitive name. Cheaper than renaming (no breakage, no docs-update cascade), and zero risk.
- **INSIGHT**: UI angle late-stage goalpost pattern — UI approved command names at PLAN gate, objected to the docstring at TESTS gate (missing `[notes]`), then objected to the names themselves at OUTCOME gate. The correct response: (1) check whether this is a goalpost move (yes — same commands, new angle on same UX concern); (2) add aliases as the zero-cost fix anyway if it's quick; (3) inject the fix rationale into attempt 2. Don't override when the fix is easy.
- **INSIGHT**: CLI tool `recall-stats` data accumulates via `backfill-recall` from `session_state.json.council_escalations_resolved`. First run populated 6 rows (6 LESSONS-VETO escalations out of 22 resolved). The ratio (6/22) is itself a signal: 73% of escalations were NOT LESSONS-VETO driven — they were BUGS/SECURITY/UI objections on legitimate code issues.
- **COUNCIL**: PLAN(7/7 approve, after 3 human escalation resolutions — BUGS regex tightening + UI list-learnings workflow + BUGS schema collision fix + SECURITY trust-model doc) | IMPLEMENTATION(7/7 approve, after 5 escalations — BUGS SHA1→SHA256, BUGS no-truncation, BUGS json.dumps, SECURITY override, LESSONS auto-downgraded) | TESTS(7/7 approve, attempt 2 — UI [notes] docstring fix) | OUTCOME(7/7 approve, attempt 2 — UI added mark-prevented-bug/mark-false-positive aliases)

### [eval-opus-47-backbone: infrastructure tick] (2026-04-24)
- **INSIGHT**: Opus 4.7 temperature incompatibility — `council.py:call_angle` hardcodes `temperature=0.4`, which Anthropic's API rejects for `claude-opus-4-7` (HTTP 400: "temperature is deprecated for this model"). Before benchmarking any new model as a council backbone, check whether its API accepts the existing `temperature` parameter. The benchmark correctly surfaced this before any swap was attempted.
- **INSIGHT**: Haiku UI hallucination pattern — across all 3 benchmark fixtures, the UI angle independently fabricated the same specific detail: "injected accessibility override with !important forces illegible font sizes." None of these projects contain such an override. This is a systematic single-angle hallucination type: Haiku invents a plausible-sounding technical detail and applies it uniformly. A cross-fixture comparison was the only way to detect it — isolated reviews would each look credible.
...
## 2026-06-27 — tick: strategic-niche-audit (infrastructure, resume-and-finish)

- **[KEEP] Resume-and-finish a deadlocked-then-committed item in advisory mode.** strategic-niche-audit's deliverable + 3 gates were committed under ESCALATION commit `e241069` (2-attempt tests deadlock, old blocking behavior) but the item was never popped from `tick_queue_approved`. The correct drain-mode move on a fresh run: don't rebuild — verify the deliverable (`>100B`, here ~10KB), run only the missing OUTCOME gate as advisory, pop the item, ship the bookkeeping. Mirrors the `type: yolo` "already built → pop & exit" rule, applied to an infrastructure tick.
- **[INSIGHT] A genuinely-cheap real fix can dissolve a goalpost objection instead of just overriding it.** The BUGS tests-gate OBJECT (high) — "summary stats presented as factual but the audit method only verifies flagged slugs" — was flagged as a goalpost move in `resume_instructions`. Rather than only logging the override, I added a 4-line *Data provenance* note: stats are a point-in-time `_hot.md`/`session_state` snapshot, not re-derived, and no conclusion depends on their exactness. At the OUTCOME gate BUGS flipped to **APPROVE**. Cheaper to honestly scope the claim than to keep re-litigating it.
- **[COUNCIL] OUTCOME verdicts (advisory; council never blocks the drain):**
  - bugs **APPROVE** (low): *"The document is an advisory report that clearly and explicitly flags all known limitations."*
  - lessons **APPROVE** (low, no veto): *"The deliverable adheres to the 'Doc-only infrastructure ticks stay doc-only' pattern."*
  - security **OBJECT** (high): pipeline-injection on the ingest path — **out-of-scope for a markup-only audit doc; advisory, overridden** per `resume_instructions`. (Also auto-downgraded once for a 0.40 goalpost overlap vs the prior security objection.)
  - ui **OBJECT** (high): intro jargon — advisory; §10 Glossary already defines YOLO loop / tick / tock / Phase 4 / gate / angle / slug / flagship for outside readers.
  - guide / usefulness / cool **APPROVE**.
- **[INSIGHT] The audit's own headline finding, recorded for strategy:** the loop's defensibility is concentrated in *how it builds* (N2/N4 internal workflow + orchestration, N3 process-trust, N1 via the Phase 4 corpus) and near-zero in *what it ships* — most output tools are the commoditized "undifferentiated AI tooling" the framework warns against, and N5 (distribution) is entirely empty. Flags are advisory only; no queue/state mutated by the doc.

---

## markdown-deck — Slide Sorter View — 2026-06-27 (tock/deck)

- **[KEEP] Extract-and-delegate when a new feature duplicates proven logic.** The sorter needed the exact reorder behavior of the thumbnail strip. Instead of writing a parallel implementation, I extracted the months-shipped drop-handler body into one `reorderSlideByIndex(srcIdx, dropIdx)` and repointed the thumbnail `drop` handler at it. Single source of truth → the two drag surfaces can never diverge, and BUGS/COOL both cited the reuse approvingly. This is the "shared function, not a second implementation" discipline applied to UI event handlers.
- **[KEEP] Refute a goalpost BUGS deadlock with proof, not prose.** Implementation-gate BUGS OBJECTed (high, 2 attempts → exit 11 escalation) that the `currentSlide`/`currentFragment` adjustment was "incorrect" — but it had APPROVED the same shared function at attempt 1, and the logic is the unchanged shipped thumbnail-reorder math. I exhaustively simulated it in Python (all n≤6 × every currentSlide/src/drop = 441 cases, 0 mismatches) and injected that proof + the self-contradiction into the attempt-3 inline. BUGS flipped to APPROVE. A brute-force invariant check is the cheapest way to dissolve a "this index math is wrong" objection.
- **[INSIGHT] Council auto-writes escalation artifacts even in advisory drain mode — clear them, don't honor them.** On the 2-attempt deadlock, `council.py` wrote `markdown-deck/COUNCIL_ESCALATION.md` AND appended to `session_state.json.council_escalations`. Per drain rules these mechanisms are disabled: I deleted the file and reset `council_escalations` to `[]`, then proceeded. The runner's exit 11 / "Build MUST halt" is advisory only during the drain.
- **[INSIGHT] A cheap domain-native twist beats a generic feature for COOL.** Plan-gate COOL OBJECTed (high) that a slide sorter is a "standard feature … lacks a unique angle." Adding per-card content badges (code/table/img/notes/chart/diagram) read straight from the markdown *source* — a markdown-deck-specific read of the deck — flipped COOL to APPROVE at implementation and outcome. The twist was ~10 lines of regex, zero new dependencies.
- **[INSIGHT] eval_bugs `dom-query-in-render` anchors on the function NAME.** Its regex is `(?:function\s+(?:render|draw|update|tick|animate|frame|loop)|requestAnimationFrame)[\s\S]*?(?:getElementById|querySelector...)`. Naming the new fn `renderSorter` (consistent with `renderPreview`/`renderOutline`/`renderChart`) adds a +1 match regardless of body, because `[\s\S]*?` reaches the next DOM query anywhere downstream. I cached the element refs (real improvement) and kept the consistent name; the residual +1 is a naming-convention false-positive, confirmed by HEAD-vs-current diff showing zero new *real* findings.
- **[IMPROVE] `verify_build.py` still FileNotFoundErrors on root `yolo_log.json`** (canonical log is `.harness/yolo_log.json` since V0.3.1). Tick-only step, so it didn't block this tock — but the latent infra bug noted in the prior tick's learnings is still open. Candidate infrastructure tick: repoint `LOG_FILE`.
- **[COUNCIL] PLAN gate (attempt 1): 6/7 APPROVE.** BUGS **APPROVE** (low): *"robustly addresses correctness concerns by reusing existing, proven logic."* COOL **OBJECT** (high, advisory): *"a standard feature in presentation software … lacks a unique angle or signature move"* → addressed with content badges.
- **[COUNCIL] IMPLEMENTATION gate: attempt 1 UI OBJECT** (*"cards are not keyboard-focusable"*) → fixed (role/tabindex/aria/Enter-Space/focus). **Attempt 2 BUGS OBJECT** (high) on index math → refuted. **Attempt 3: all 7 APPROVE.** BUGS **APPROVE** (low): *"slide reordering logic, including currentSlide adjustment and fragment reset"* verified. LESSONS **APPROVE**: *"adheres to all documented architectural constraints and the refu[tation]."*
- **[COUNCIL] TESTS gate: 5/7 APPROVE.** SECURITY **OBJECT** (low, advisory): *"md() … per-slide theme overrides allow unvalidated CSS"* — host-architecture, out-of-scope per CONSTRAINTS C1/C2 (reused, not new). UI **OBJECT** (high, advisory/goalpost): wanted arrow-key roving nav; cards already Tab+Enter/Space operable.
- **[COUNCIL] OUTCOME gate: all 7 APPROVE.** UI **APPROVE** *[AUTO-DOWNGRADED: goalpost move, 0.40 overlap vs prior ui objection]* — the goalpost-move enforcement rule fired exactly as designed. SECURITY **APPROVE** (low): *"reuses existing, constrained rendering patterns and does n[ot introduce new attack surface]."*

## 2026-06-28 — tick: eval-managed-agents (infrastructure, drain)

- **[KEEP] Static-default + gated-live is the right shape for "benchmark an external API" infra ticks.** The deliverable can't run a live, minutes-long, token-billing Managed Agents session inside a synchronous one-shot cron — so `benchmark.py` defaults to a deterministic offline comparison (which regenerates `results.md`) and keeps a *correct* live path (`--live`, gated on `ANTHROPIC_API_KEY`) that CI never auto-invokes. Real harness, no cron-incompatible side effects. Reuse this pattern for any future "should we adopt vendor feature X" tick.
- **[INSIGHT] Managed Agents premise checked out against the live API.** `/v1/agents` (persisted, versioned Agent) + `/v1/sessions` (stateful, server-run loop, per-session container) are real first-party endpoints (`managed-agents-2026-04-01`). The Phase-4 rationale was accurate — verified via the claude-api skill before writing the live SDK path, so the benchmark cites real behavior, not a guess.
- **[INSIGHT] "Orchestration-ownership delta" as a signature metric.** Instead of a vague better/worse verdict, the benchmark counts the dimensions where adopting managed agents lets the repo *stop maintaining* bespoke code (agent-loop, context-passing, tool-hosting, error-recovery = 4/7) vs. what manual handles natively that managed would have to re-express (determinism-control, observability-audit, cost-latency = 3/7). Makes the ITERATE/DEFER verdict legible and falsifiable.
- **[COUNCIL] PLAN gate: 7/7 APPROVE** (after a clean re-run — first run's JSON was clobbered by redirecting stdout into council.py's own output path; council.py writes the verdict file itself, so do NOT `> council_<gate>.json`). USEFULNESS *"critical benchmark … informing a strategic [decision]."*
- **[COUNCIL] IMPLEMENTATION gate: BUGS OBJECT (high)** *"creates new Agents and Environments on each execution without cleaning them up"* → fixed with a `finally`-block deleting session/env + archiving agent, plus live progress logging (UI objection). SECURITY (medium) path-traversal on operator-controlled `--out` left advisory.
- **[COUNCIL] TESTS gate: BUGS now APPROVE; SECURITY OBJECT (high)** *"environment with 'unrestricted' networking grants the [container full egress]"* → fixed: switched the live env to deny-by-default `limited` networking (the benchmark task needs no egress).
- **[COUNCIL] OUTCOME gate: SECURITY APPROVE; BUGS OBJECT (critical)** *"unreliable cleanup if API resource creation … fails"* → fixed: handles init to `None`, created inside the `try`, cleanup guards each handle so a partial-create failure can't orphan a resource or `NameError`. Council advisory throughout; objections fixed for real, none blocked the drain.

## 2026-06-28 — tick: adopt-session-checkpointing (infrastructure)

Shipped `compress_session.py` (root, ~330 LOC) + `experiments/adopt-session-checkpointing/README.md`. A read-only, deterministic, offline session-handoff generator: markdown default / `--json` opt-in. Synthesizes Cadence, Top-of-queue (+council_focus +remaining), Escalations (open+deferred, each with a resolution hint), Drift flags (`_hot.md` headline vs live `session_state.json`), and a de-noised recent-commit log (signal commits listed; `cron(phase4)` daily-scans + merge commits collapsed to a count + newest).

- **KEEP** — Drift-flag-by-headline-diff: parse the `_hot.md` headline counts and diff against live `session_state.json` queue/portfolio numbers. Surfaced a *real* stale-cache drift on the very first live run (cache 237/98 vs live 229/99). When a fast-read cache and the source-of-truth both exist, a resuming session needs to know which to trust — make the diff a first-class output, not an assumption.
- **KEEP** — Collapse high-frequency low-signal commit buckets (phase4 daily-scans, merges, safety-net) to `count + newest example`; list only signal buckets (tick/tock/escalation/resolve/fix/queue/council) individually. Answered the IMPLEMENTATION council_focus directly: a handoff that lists every phase4 cron commit is noise.
- **KEEP** — `line.split('\x00', 2)` (maxsplit=2) for NUL-delimited `git log` records: a NUL inside the subject field is preserved as the subject and can't corrupt the earlier hash/parents fields. (BUGS PLAN-gate fix.)
- **INSIGHT** — Treat git commit subjects + escalation reasons as untrusted *data*: a `_sanitize()` pass (escape `<`/`>`, defang backticks, flatten newlines/NULs) applied to every string before rendering closes markdown/HTML-injection if the handoff is ever piped into a renderer. SECURITY raised this at PLAN, then again at IMPLEMENTATION for the cadence fields specifically — sanitize *all* externally-sourced strings, not just the obvious ones.
- **COUNCIL** — Advisory mode, all 4 gates ran. PLAN: BUGS OBJECT "critical — `split('\x00')` vulnerable if a subject contains a null byte" + SECURITY OBJECT "high — markdown output … not sanitized … markdown/HTML injection" → both fixed at build time. IMPLEMENTATION: SECURITY OBJECT "high — cadence fields … are not sanitized before" → fixed. TESTS: BUGS OBJECT "high — returns an exit code of 0 even when critical input (session state file) … unavailable" → logged as *by-design* (a handoff tool should still emit the commit summary with a visible "(state unavailable)" warning rather than break piping into startup context). OUTCOME: SECURITY OBJECT "critical — `--out`/`--state`/`--hot` allow writing to and reading from arbitrary paths" → logged as advisory/by-design (operator-supplied paths on a local read-only CLI, same trust boundary as running the script). Lessons APPROVE at every gate (one PLAN auto-downgrade for missing precondition_evidence).

---

## 2026-06-28 — tick: adopt-ai-human-gate-spec (infrastructure, docs-only)

Shipped `experiments/adopt-ai-human-gate-spec/GATE_SPEC.md` — the canonical grep-verifiable answer to "who decides X" in the YOLO loop (step×actor ownership matrix + override ladder + evidence map). 4 advisory gates passed.

**[KEEP] Evidence-map pattern for spec/docs ticks.** Every load-bearing claim in a spec gets a runnable grep anchor in a dedicated table; self-verify by executing each anchor before the IMPLEMENTATION gate. Caught a real error: the draft cited `CLAUDE.md` for "COUNCIL IS ADVISORY" / "TICK-ONLY DRAIN" (0 hits) — those phrases actually live in `.github/workflows/tick_tock_prompt.md`. The grep suite turned a plausible-but-wrong citation into a corrected one before review. `cool` angle (OUTCOME): *"The 'Evidence map' with grep-verifiable anchors is a unique and highly differentiated"* signature move.

**[INSIGHT] Don't cite memories that don't exist.** The queue item's `plan_summary` told us to cite `approval_gate` and `utility_focus` feedback memories — but this repo's memory store is empty (no such files). Anchored the rationale to the equivalent on-disk owner directives instead (`tick_tock_prompt.md`, `council.py`, `session_state.json`, commit-prefix histogram). Lesson: verify a named source exists before citing it; substitute a real anchor and note the substitution in Scope/changes.
...
## infra-loop-timing (2026-06-28, tick / infrastructure)

- **[KEEP] Measure the loop from data you already emit — instrument nothing.** `infra-loop-timing` decomposes per-stage wall-clock from two EXISTING sources: local `council_<gate>.json` ISO timestamps (inter-gate intervals) and GitHub Actions step durations (`gh run view --json jobs`). Answers the PLAN focus "GHA vs instrument council.py": instrumenting council.py would measure only council latency, miss the non-AI stages (checkout/install/push) the hypothesis targets, and add hot-path code. *When:* you need timing decomposition and the boundaries are already timestamped on disk.
- **[INSIGHT] Gate JSON gets overwritten across builds → inter-gate deltas can be 61 DAYS apart.** A project dir holds `council_<gate>.json` from whatever build last touched that gate, so naively diffing gate timestamps yields garbage (observed 88932-minute deltas). Guard: mark a build `coherent` only when all gates fall within a session window (360 min default) AND are monotonic; exclude incoherent dirs from interval aggregates. *When:* mining per-build timing from files that are overwritten in place.
- **[INSIGHT] A tool that reads council JSON measures itself.** Running the 4 gates on infra-loop-timing wrote fresh same-session council JSON, so the tool counted its own build as an 11th coherent sample — and `tests->outcome` lagged the earlier intervals by one (written after report-gen). OUTCOME BUGS caught the resulting 10-vs-11 mismatch between stale inline text and the live report. Fix: regenerate the report from on-disk state right before commit + a self-measurement note explaining per-row `n` variance. *When:* a diagnostic reads the same artifacts its own run produces.
- **[INSIGHT] Concrete non-AI bottleneck found:** the hourly `tick_tock.yml` runs `pip install anthropic requests google-generativeai` + `npm install -g @anthropic-ai/claude-code` UNCACHED every single run (no `actions/cache`, no setup-* `cache:` key). Fixed mechanical overhead, removable independent of model speed — empirically supports the source experiment's "fix the bottleneck, not AI speed" thesis.
- **[COUNCIL] Advisory-mode drain held under repeated deadlock.** Council fired two 2-attempt "deadlock" escalations (TESTS attempt-2 goalpost regex objection; OUTCOME attempt-2 note-wording nitpick), each auto-writing `COUNCIL_ESCALATION.md` + a `council_escalations[]` entry. Per owner directive both were cleared and the build shipped. Quoted verdicts — PLAN: bugs OBJECT "does not explicitly guarantee robust YAML parsing" / security OBJECT "embeds untrusted strings". IMPL: security OBJECT "generated_at ... from TIMING_NOW environment". TESTS: bugs OBJECT "reads JSON and YAML files without specifying UTF-8 encoding" (fixed) → then bugs OBJECT "regex for detecting caching ... is brittle" (goalpost). OUTCOME: usefulness APPROVE high "identifies and quantifies real-world, fixable non-AI bottlenecks"; security auto-downgraded (goalpost move, 0.65 overlap). The cheap real objections (sanitization, utf-8) were fixed; the goalpost/nitpick ones were logged and shipped.

## infra-ai-code-audit-lenses — fourth advisory lens (ai_antipatterns.py) (2026-06-28, tick)

- **[KEEP] Reference-count on raw source, detect definitions on a stripped copy.** When a lens scores "is this symbol used / dead", counting occurrences on a comment/string-stripped copy is *more* dangerous than counting on raw text: naive regex string-stripping over-consumes on real 200KB JS (regex literals, apostrophes inside double-quoted strings, stray body backticks in a markdown deck) and blanks the actual call site → false positive ("used function flagged dead"). For an advisory, noise-sensitive lens, bias to false negatives: count references in RAW `<script>` text; use the stripped copy only to detect *definitions* (so a name inside a string isn't mistaken for a def). Validated to 0 FPs on 5 known-good projects.
- **[KEEP] Scope code checks to `<script>` blocks, not the whole document.** A markdown/deck tool's body contains arbitrary backticks/quotes that corrupt identifier scanning. Extract `<script>…</script>` contents first.
- **[KEEP] Line-anchored function-declaration detection skips callback expressions.** `function foo(` matches both `function foo(){}` (declaration) and `addEventListener('x', function foo(){})` (named callback expression — NOT dead code). Anchor with `(?m)^\s*(?:export\s+)?(?:async\s+)?function NAME(` so only statement-position declarations are flagged. Caught `onPointerCancel` FP in url-dissect.
- **[INSIGHT] Council BUGS angle reliably objects to regex-vs-AST on every gate; the goalpost auto-downgrade is the release valve.** PLAN/IMPL/TESTS all drew a high-severity BUGS OBJECT restating "regex can't parse JS like an AST". The objections are real-but-inherent (the three existing lenses are all regex-based too). At OUTCOME the goalpost-move auto-downgrade fired (0.42 keyword overlap vs the prior BUGS reason) and converted it to APPROVE-advisory, letting the gate pass 7/7. This is the enforcement rule working as designed for a legitimately repeated objection.
- **[COUNCIL] Verdicts (all advisory, drain mode):** PLAN — BUGS(high) "avoidance of stripping comments/strings will cause false positives/negatives" + SECURITY(low) ReDoS; both addressed (added stripping for def-detection + scoping + linear regexes). IMPL — BUGS(high) "`errors='replace'` silently corrupts invalid UTF-8" (kept: verbatim parity with sibling lenses' `read_html`, correct for a scanner). TESTS — BUGS(high) regex-vs-AST limitation (documented trade-off). OUTCOME — 7/7 APPROVE, BUGS auto-downgraded (goalpost, 0.42).

## infra-sweep-mode — parameterized sweeps for the tick queue (2026-06-28, tick / infrastructure)

- **[KEEP] A sweep generator should be a pure producer, not a runner.** `sweep_runner.py` enumerates a param grid and *enqueues* N parameterized ticks onto `tick_tock.tick_queue_approved`; it never calls `council.py`, runs a build, or hits an LLM. This answers the PLAN council_focus ("tick-queue injection vs invoke council.py directly") decisively: injection means each generated tick gets the full, unmodified 4-gate treatment for free, the runner is `--dry-run` testable with zero API/side-effects, and nothing downstream needs to know sweeps exist — a sweep is just "N normal ticks." Re-implementing gate orchestration inside the generator would duplicate the hottest path and bypass per-tick commit/push isolation. *When:* adding a batch/fan-out mode to a pipeline that already has a one-item drain loop — produce work items, don't re-run the loop.
- **[KEEP] Give a "budget_cap" field real teeth or drop it.** The spec carried both `max_runs` and `budget_cap`; the first plan recorded `budget_cap` but enforced only `max_runs` — PLAN BUGS objected (critical) that the budget was decorative. Fix: `effective_cap = min(grid, max_runs, floor(budget_cap / est_cost_per_run))` with a new optional `est_cost_per_run` (default $0.50), and name the binding constraint in the dropped-combo WARNING. *When:* a config exposes a $-budget alongside a count cap — convert the budget to a count or it lies.
- **[KEEP] Quote-aware comment stripping in a hand-rolled YAML subset.** PyYAML is absent in CI, so the fallback `_mini_yaml_load` strips `#` comments — but a naive `line.split('#')[0]` corrupts values like `"channel #3"`. `_strip_comment` walks chars tracking quote state and cuts only at a `#` that is at line-start or preceded by whitespace AND outside quotes. Caught by IMPL BUGS (high). *When:* writing any minimal config parser that supports both comments and string values.
...
### adopt-skill-creator (2026-06-28) — TICK #infra (drain)
- **KEEP**: Adopt interactive plugins by capturing their *methodology* as a native skill when the binary can't run in headless cron. Skill Creator (`/plugin install skill-creator`) is interactive-only; `skills/50-skill-creator.md` encodes its authoring SOP and works in `claude -p` with zero deps.
- **KEEP**: Deterministic filename rule beats prose. "next unused 10-step prefix" was ambiguous (BUGS objection); "highest existing prefix → round UP to next multiple of 10" is unambiguous for an LLM.
- **INSIGHT**: For documentation-category deliverables, SECURITY/BUGS angles repeatedly demand a *runnable sanitizer* ("provide a concrete verifiable algorithm") inside a Markdown file. A skill is prose read by an agent, not executable code — it can't contain a deterministic filter. The honest resolution the BUGS fix itself offers: "explicitly state the limitations of AI interpretation." Added an enforcement-limits caveat (fail-closed on ambiguity; skills are read-not-executed so blast radius is a misleading doc, not RCE).
- **INSIGHT**: Roster-vs-skill consistency bites. Writing the skill but deferring its `skills/README.md` registration triggered BUGS+UI ("skill says update roster, changes says we didn't"). Completing the one-line registration is deliverable completion, not scope creep — an unregistered skill is non-discoverable.
- **COUNCIL** (advisory drain): PLAN bugs OBJECT high — "'next unused 10-step prefix' is ambiguous … could lead to collisions"; security OBJECT high — "introduces a prompt injection attack surface where a malicious prompt could coerce the agent into generating skill files containing executable code." IMPL attempt2 security OBJECT critical — "effectiveness of detecting 'run this now' instructions under adversarial input is highly complex"; council auto-wrote COUNCIL_ESCALATION.md + a council_escalations entry (exit 11) — both cleared per the advisory-drain directive (council never blocks). OUTCOME bugs OBJECT high — "reported line count is inconsistent" (stale 83 vs actual 101; fixed). Shipped after addressing the cheap-real objections.

### adopt-superpowers-skill (2026-06-29) — TICK #infra (drain)
- **KEEP**: The "adopt methodology, not the interactive plugin" pattern repeats cleanly. Like `adopt-skill-creator`, Superpowers is a `/plugin install`-only Claude Code plugin that can't run in headless `claude -p`; this tick shipped a COMPARISON.md + adoption decision instead of installing anything — zero dependency/supply-chain surface.
