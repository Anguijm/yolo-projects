...
- **Patch 1 (Parse retry)** — `call_angle(angle, user_message, _retry=False)`: after `Verdict.from_raw`, if the returned verdict has `parse_failed=True` and we haven't already retried, append stricter JSON instructions and recurse once with `_retry=True`. Prevents phantom OBJECTs from transient truncation. Detection uses a new `parse_failed: bool = False` field on `Verdict` (set to True by `from_raw` on `JSONDecodeError`) rather than string-matching the reason text — per BUGS low-severity critique raised on fix-council-enforcement PLAN escalation 6 (2026-04-21): "string comparison will break silently if the error message changes."
